...
class UsuariServiceTest {
    ...
    public void TestFindUserById() {
        RegisterUserDTO usuari = new RegisterUserDTO(
                "demo1",
                "demo1",
                "demo1",
                "demo1",
        ...
        ResponseEntity<ResponseDTO> response = loginService.registerUsuari(usuari);
        Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
        loginService.login(usuari.getUsername(), usuari.getPassword());

        Optional<Token> token = tokenRepository.findByUsername("demo1");
        Optional<Usuari> foundUsuari = usuariRepository.findUserByUsername("demo1");
        response = usuariService.findById(foundUsuari.get().getId(), token.get().getToken());
        Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
    }
